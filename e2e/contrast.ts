 ...
 *
 *  - text over a surface axe declines to resolve. This page has no gradients,
 *    but it is built out of `color-mix()`: `.cl-hero-why` is a
 *    `color-mix(in oklab, var(--pq-color) 6%, transparent)` wash over the
 *    canvas, every lit `.mech-lock` and `.mech-out` tints `--surface2` with
 *    `--success`/`--danger`, the two `.sig-seg` halves and every `.badge` draw a
 *    `color-mix()` edge, and `.primer-caption code` / `.method-note code` sit on
 *    a `color-mix(in srgb, var(--composite-ok) 12%, transparent)` wash. Chromium
 *    reports all of those to `getComputedStyle` unresolved, so axe files the
 *    text on them under `incomplete` and a violations-only gate measured none of
 *    the lit states this lab exists to show.
 ...
 *    at `.85` and `.btn:disabled` at `.4` — and the disabled buttons matter,
 *    because eight of the nine controls on this page ship disabled until a
 *    keypair exists, which is the first state the gate scans.
 *
 ...
 *    here are `.hex-display` (`max-height: 80px; overflow-y: auto`, which the
 *    3,373-byte composite signature overflows the moment "Show full signature"
 *    is pressed), `.sig-seg` (`overflow: hidden` around a hex run), `.table-wrap`
 *    (`overflow-x: auto` around a `min-width: 480px` comparison table, which
 *    clips at 380px) and the TLS-registry `<pre>` (`overflow-x: auto`). Content
 ...
 *    never laid down. Live here: `.censor`, the redaction bar standing in for
 *    each private key, is a `--danger` block whose block-glyph text is painted
 *    `color: transparent` so nothing can be read off it.
 *
 *  - SVG PAINTS IN DOCUMENT ORDER, SO SIBLINGS CAN BE THE BACKGROUND. This lab
 *    draws its one figure — the two-lock AND gate — in HTML and CSS grid, and
 *    the only SVG on the page is the shared header's two icons, which carry no
 *    `<text>`. The rule is kept because the alternative is a helper that
 ...
 *
 * FIRST, generated content. The walk iterates ELEMENTS, and a `::before` is not
 * one — nor is it one to axe's `color-contrast` rule. This page has two, both
 * measured by hand from real screenshot pixels instead: `.steps li::before`
 * (content: '▶', the marker on each of the four signing steps) painted
 * `--composite-ok`, and `.mech-arrows span::before` (content: '→') painted
 * `--text-muted`. Both are decoration beside text that carries the same meaning,
 * and both clear 4.5:1 on the surface they sit on in both themes.
 *
 * SECOND, the two `.sig-flip-swatch` / `.sig-flip` marks. `.sig-flip` is an
 * element and so IS walked for its own ink, but the thing that has to be
 * perceivable about it is its FILL against the hex around it — SC 1.4.11, not
 * 1.4.3 — and no oracle here measures that. It is measured by hand and tokenised
 * as `--flip-fill` / `--flip-ink` for exactly that reason.
 */
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     * into surfaces with `color-mix()` in both `oklab` and `srgb` — the hero's
     * "why it matters" aside is `color-mix(in oklab, var(--pq-color) 6%,
     * transparent)`, every lit lock is `color-mix(in srgb, var(--success) 12%,
     * var(--surface2))` — and Chromium reports those
     ...
     * fabricates failures (a dark-on-light key card read as dark-on-dark) and, far
     ...
     * two `.sr-only` spans reading "withheld — never leaves the browser", each
     * the accessible twin of an `aria-hidden` `.censor` redaction bar, would be
     * measured as `--text` over the `--danger` block beside them. Deliberately
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. This page's disclosures are `.method-note` ("How these
      // simulations stay honest") and `.why-shor-more` ("A bit more depth"),
      // whose open states are real states the gate scans explicitly.
     ...
     * it, the AND-gate diagram's `aria-hidden` lock emoji (🔒/🔓) — and the
     * matching `.lock-icon` in every verify row — is measured as if its glyph
     * took the element's inherited `color`, but an emoji paints in its own
