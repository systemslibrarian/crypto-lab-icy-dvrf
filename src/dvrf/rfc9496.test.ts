...
import { Point, bytesToHex } from './group.ts'
...
describe('RFC 9496 A.1 — multiples of the generator (16 KATs)', () => {
  GENERATOR_MULTIPLES.forEach((expected, k) => {
    it(`encodes ${k}·B correctly`, () => {
      const p = k === 0 ? Point.ZERO : Point.BASE.multiply(BigInt(k))
      expect(bytesToHex(p.toBytes())).toBe(expected)
      // Round-trip: decoding the canonical bytes yields the same point.
      expect(Point.fromHex(expected).equals(p)).toBe(true)
    })
  })
})

describe('RFC 9496 A.2 — invalid encodings are rejected (29 KATs)', () => {
  INVALID_ENCODINGS.forEach((hex, i) => {
    it(`rejects invalid encoding #${i + 1}`, () => {
      expect(hex).toHaveLength(64)
      expect(() => Point.fromHex(hex)).toThrow()
    })
  })
})
